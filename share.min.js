// https://github.com/carrot/share-button/blob/v0.5.0/build/share.min.js
...
(function() {
window.ShareConfig = function(shareName) {
    function logNetwork(name) {
        return function() {
            amplitude.logEvent("share network clicked", {'network': name, 'location': shareName});
        }
    }
    return {
        ...
        networks: {
            facebook: {
                after: logNetwork('facebook')
            },
            google_plus: {
                after: logNetwork('google')
            },
            twitter: {
                after: logNetwork('twitter')
            },
            pinterest: {
                after: logNetwork('pinterest')
            },
            email: {
                after: logNetwork('email')
            }
        },
    };
};

var config = ShareConfig('video');
config.ui = { button_text: '', flyout: 'top left'},
new Share('.video-share', config);

function logShare(event) {
    amplitude.logEvent("share clicked", {'location': 'video'});
    event.preventDefault();
    return false;
}
var button = document.querySelector('.status .share label');
button.addEventListener('mouseup', logShare);
button.addEventListener('touchup', logShare);
})();
